defmodule Bamboo.PostmarkAdapter do
  ...
  @send_batch_email_path "email/batch"
  defmodule ApiError do
    ...
    def exception(%{params: params, response: response}) do
      filtered_params =
        List.wrap(params)
        |> Enum.map(&Bamboo.PostmarkAdapter.json_library().decode!(&1))
  ...
  def deliver(emails, config) do
    ...
    params =
      emails
      |> Enum.map(fn email ->
        email |> convert_to_postmark_params() |> json_library().encode!()
      end)
      |> case do
        [param] -> param
        params -> params
  ...
  defp api_path([_ | _]), do: @send_batch_email_path
